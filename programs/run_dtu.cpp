...
static void print_usage(const char* prog) {
  std::cerr
      ...
      << "  --aiding {gnss|pars}        aiding source (default gnss)\n"
...
void run_estimation(const Data& d, const Options& opts) {
  ...
  // Only used with --aiding pars; DF/CS are OFF in the default DTU export, so
  // update this from the latest R_radio_ned before enabling PARS.
  Eigen::Matrix3d R_rn;
  R_rn << 0.3545, -0.8725, -0.3363, 0.6295, 0.4886, -0.6041, 0.6914, 0.0024,
      0.7225;
  ...
  for (int64_t idx = 1; idx < N; ++idx) try {
      ...
      // Which measurements fire this tick. For pars we bootstrap with GNSS
      // until switch_time, then hand over to the DF/CS aiding.
  ...
  const std::string out_file =
      opts.output_dir + "dtu_" + bias_tag + se23_tag + ".csv";
...
int main(int argc, char** argv) {
  ...
  const char* aiding_name =
      opts.aiding == Aiding::Gnss ? "GNSS (DGPS)" : "PARS (DF+CS)";
