...
/**
 *  @file  ManifoldPreintegrationT.h
 *  @author Luca Carlone
 ...
 * This corresponds to the original RSS paper (with one difference: V is
 * rotated)
 */
template <typename Bias = imuBias::ConstantBias>
class GTSAM_EXPORT ManifoldPreintegrationT : public PreintegrationBaseT<Bias> {
 protected:
  using PreintegrationBaseT<Bias>::biasHat_;
  using PreintegrationBaseT<Bias>::deltaTij_;
  using PreintegrationBaseT<Bias>::p_;
  using PreintegrationBaseT<Bias>::p;

 public:
  using Params = typename PreintegrationBaseT<Bias>::Params;
  using PreintegrationBaseT<Bias>::deltaTij;
   ...
   * velocity at time i, see deltap+, in [2] Note: velocity
   * is now also in frame i, as opposed to deltaVij in [2]
   */
  NavState deltaXij_;
  Matrix3 delRdelBiasOmega_;  ///< Jacobian of preintegrated rotation w.r.t.
                              ///< angular rate bias
  Matrix3 delPdelBiasAcc_;    ///< Jacobian of preintegrated position w.r.t.
                              ///< acceleration bias
  Matrix3 delPdelBiasOmega_;  ///< Jacobian of preintegrated position w.r.t.
                              ///< angular rate bias
  Matrix3 delVdelBiasAcc_;    ///< Jacobian of preintegrated velocity w.r.t.
                              ///< acceleration bias
  Matrix3 delVdelBiasOmega_;  ///< Jacobian of preintegrated velocity w.r.t.
                              ///< angular rate bias

  /// Default constructor for serialization
  ManifoldPreintegrationT() { resetIntegration(); }
 ...
 public:
  ...
  ManifoldPreintegrationT(const std::shared_ptr<Params>& p,
                         const Bias& biasHat = Bias())
      : PreintegrationBaseT<Bias>(p, biasHat) {
    resetIntegration();
  }
  /// @}
  ...
  /// @name Testable
  /// @{
  bool equals(const ManifoldPreintegrationT& other, double tol) const;
  /// @}
  ...
  /// possibly sensor pose NOTE(frank): implementation is different in two
  /// versions
  ...
  virtual std::shared_ptr<ManifoldPreintegrationT> clone() const {
    return std::shared_ptr<ManifoldPreintegrationT>();
  }
 ...
 private:
  ...
  void serialize(ARCHIVE& ar, const unsigned int /*version*/) {
    ...
    ar& BOOST_SERIALIZATION_BASE_OBJECT_NVP(PreintegrationBaseT);
