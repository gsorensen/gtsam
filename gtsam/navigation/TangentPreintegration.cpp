...
void TangentPreintegration<Bias>::resetIntegration() {
  deltaTij_ = 0.0;
...
//------------------------------------------------------------------------------
template <typename Bias>
bool TangentPreintegration<Bias>::equals(const TangentPreintegration& other,
                                         double tol) const {
...
template <typename Bias>
Vector9 TangentPreintegration<Bias>::UpdatePreintegrated(
  ...
  const Vector3 w_tangent =  // angular velocity mapped back to tangent space
      local.InvJacobian().applyRight(w_body, A ? &w_tangent_H_theta : 0,
                                     C ? &invH : 0);
  ...
  if (A) {
    ...
    const Matrix3 a_nav_H_theta =
        nRb * skewSymmetric(-a_body) * local.rightJacobian();
...
//------------------------------------------------------------------------------
template <typename Bias>
void TangentPreintegration<Bias>::update(const Vector3& measuredAcc,
...
//------------------------------------------------------------------------------
template <typename Bias>
Vector9 TangentPreintegration<Bias>::biasCorrectedDelta(
...
//------------------------------------------------------------------------------
template <typename Bias>
Vector9 TangentPreintegration<Bias>::Compose(const Vector9& zeta01,
...
//------------------------------------------------------------------------------
template class GTSAM_EXPORT TangentPreintegration<imuBias::ConstantBias>;
